...
# Install Ansible for playbook generation and Java for the sas-orchestration tool dependency
RUN apt-get update && \
    apt-get install -y openjdk-8-jdk-headless ansible && \
    rm -rf /var/lib/apt/lists/*

# TODO: use dep or go mod to handle deps.
...
# Create a user in the docker group
ARG USER_UID=1000
ARG DOCKER_GID=997
...
# Run the sas-container-recipes binary
