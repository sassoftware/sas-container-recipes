...
// Grab the 'util/single-programming-only/Dockerfile' and append any addons
...
func buildProgrammingOnlySingleContainer(order *SoftwareOrder) error {
	...
	resourceDirectory := "util/single-programming-only"
	...
	err = container.AddFileToContext("", "entrypoint", []byte(dockerfile))
	if err != nil {
		return err
	}

	// Open the context payload so it can be passed to the Docker client
...
func (order *SoftwareOrder) LoadPlaybook(progress chan string, fail chan string, done chan int) {
	...
	if err != nil {
		fail <- "[ERROR]: The package `ansible` must be installed in order to generate Kubernetes manifests after the build process."
	}
...
`,
	...
	// Write a temp file
	varsFilePath := order.BuildPath + "sas-temp-vars.yml"
	...
	// TODO: clean this up
	playbook := `
# DO NOT EDIT - This is generated by order.go
---
