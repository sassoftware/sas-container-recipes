...
class TestSmoke():
    ...
    def test_04_programming_process_running(self):
        """
        Ensure programming process is running
        """
        cmd = "exec {} -- ps xh c".format(self.TARGET_POD_NAME)
        status, result = self.kubeutilobj.kubectl(cmd, self.NAMESPACE)
        print(status, result)
        assert status == 'Success' and b"programming" in result["output"]
    ...
    def test_15_check_package_installed(self):
        """
        Ensure common package is installed
        """
    ...
    def test_16_check_package_installed(self):
        """
        Ensure common package is installed
        """
