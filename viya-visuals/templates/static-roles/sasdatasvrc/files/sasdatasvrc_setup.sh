...
# Define defaults for the serice's hba configuration
...
function create_user {
    ...
    if ${SASHOME}/bin/psql postgres -h ${SASPOSTGRESRUNDIR} -p ${SASPOSTGRESPORT} -U ${SASPOSTGRESOWNER} -tAc "SELECT 1 FROM pg_roles WHERE rolname='${dbuser}'" | grep -q 1; then
...
function create_database {
    ...
    if ${SASHOME}/bin/psql -h ${SASPOSTGRESRUNDIR} -p ${SASPOSTGRESPORT} -U ${SASPOSTGRESOWNER} -lqt | cut -d \| -f 1 | grep -qw ${SAS_DBNAME}; then
      ...
      su - -c "${SASHOME}/bin/createdb -h ${SASPOSTGRESRUNDIR} -p ${SASPOSTGRESPORT} -U ${SASPOSTGRESOWNER} -O ${SASPOSTGRESOWNER} -E UTF8 -T template0 --locale=en_US.utf8 ${SAS_DBNAME}" ${SASPOSTGRESOWNER}
    fi

    echo_line "grant all privileges on ${SAS_DBNAME} to ${SAS_DEFAULT_PGUSER}"
    # -c = run only single command (SQL or internal) and exit
    su - -c "${SASHOME}/bin/psql -h ${SASPOSTGRESRUNDIR} -p ${SASPOSTGRESPORT} -U ${SASPOSTGRESOWNER} postgres -c 'GRANT ALL PRIVILEGES ON DATABASE \"${SAS_DBNAME}\" TO \"${SAS_DEFAULT_PGUSER}\"'" ${SASPOSTGRESOWNER}

    if [ "${SAS_INSTANCE_PGUSER}" != "${SAS_DEFAULT_PGUSER}" ]; then
        echo_line "grant all privileges on ${SAS_DBNAME} to ${SAS_INSTANCE_PGUSER}"
        # -c = run only single command (SQL or internal) and exit
        su - -c "${SASHOME}/bin/psql -h ${SASPOSTGRESRUNDIR} -p ${SASPOSTGRESPORT} -U ${SASPOSTGRESOWNER} postgres -c 'GRANT ALL PRIVILEGES ON DATABASE \"${SAS_DBNAME}\" TO \"${SAS_INSTANCE_PGUSER}\"'" ${SASPOSTGRESOWNER}
    fi
}
...
su - -c "${SASHOME}/bin/pg_ctl -o '-c config_file=${SASPOSTGRESCONFIGDIR}/postgresql.conf -c hba_file=${SASPOSTGRESCONFIGDIR}/pg_hba.conf' -D ${PG_DATADIR} -w -t 30 start" ${SASPOSTGRESOWNER}
...
REPL
    ...
    # Run the script
    su - -c "${SASHOME}/bin/psql -h ${SASPOSTGRESRUNDIR} -p ${SASPOSTGRESPORT} -U ${SASPOSTGRESOWNER} postgres < ${SASPOSTGRESCONFIGDIR}/setup-replication.sql" ${SASPOSTGRESOWNER}
  fi
else
